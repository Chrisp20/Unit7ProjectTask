//
//  Task.swift
//Edited by Christopher Petit on 10/29/2023
...
extension Task {
    ...
    func save() {
        ...
        var exTasks = Task.getTasks()
        
        if let index = exTasks.firstIndex(where: { $0.id == self.id }) {
            // Update the existing task
            exTasks.remove(at: index) // Remove the existing task
            exTasks.insert(self, at: index) // Insert the updated task in its place
        } else {
    ...
    func removeTasks() {
        var removeTasks = Task.getTasks()
        removeTasks.removeAll { task in
            return self == task
        }
        Task.save(removeTasks)
    }
}
